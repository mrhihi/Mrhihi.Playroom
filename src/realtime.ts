...
export function createRealtime(rooms: RoomService) {
  ...
  wss.on('connection', (ws, request) => {
    ...
    ws.on('message', raw => {
      ...
      if (message.type === 'join') { const joined = rooms.addPlayer(room, message.name, message.reconnectToken); playerId = joined.id; room.clients.set(playerId, ws); send(ws, { type: 'joined', playerId, reconnectToken: joined.reconnectToken }); broadcast(); return; }
    ...
    ws.on('close', () => { const player = room.players.find(candidate => candidate.id === playerId); if (player) player.connected = false; room.clients.delete(playerId); broadcast(); });
  });
