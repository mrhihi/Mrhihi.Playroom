...
export const oldMaidGame: GameModule<OldMaidState> = {
  ...
  createState(players, config) {
    ...
    for (const id of Object.keys(hands)) hands[id] = removePairs(hands[id]);
  ...
  apply(state, { actorId, players, message }) {
    ...
    if (message.type === 'oldMaid.tease') {
      ...
      state.tease = { playerId: actorId, cardIndex: message.cardIndex, at: Date.now() };
    ...
    const active = players.map(player => player.id).filter(id => (state.hands[id]?.length ?? 0) > 0);
